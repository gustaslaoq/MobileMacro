...
function Lib.new(userCfg)
	...
	if isDemo then
		...
		self.cfg.AppSubtitle = "Biome Script UI Demo"
		self.cfg.AppVersion  = "3.8-demo"
		self.cfg.InitialPage = 6
		self.cfg.Pages = {
			{Name="Dashboard"},
			{Name="Inputs"},
			{Name="Components"},
	...
	pcall(function()
		local saved = (not isDemo) and self._loadState and self:_loadState()
		if saved then
...
function Lib:_runSplash()
	...
	elseif mode == "boot" then
		self:_beginBootSplash()
		return
	end

	self:_beginBootSplash()
	task.spawn(function()
		local tasks = cfg.SplashTasks or {"Loading...","Ready."}
		local n = math.max(#tasks, 1)
		for i = 2, #tasks do
			local label = tasks[i]
			task.wait(.38)
			self:SetBootStep(label, math.clamp(i / n, .08, .96), "startup")
		end
		task.wait(.28)
		self:FinishBootSplash("Ready")
	end)
	return
end
...
function Lib:_buildWindow()
	...
	local function computeScale()
		...
		local widthUse = isTouch and 0.88 or 0.94
		local heightUse = isTouch and 0.78 or 0.90
		local maxW = math.min(cfg.WindowWidth, vpW * widthUse)
		local maxH = math.min(cfg.WindowHeight, vpH * heightUse)

		local sx = maxW / cfg.WindowWidth
		local sy = maxH / cfg.WindowHeight
		local s  = math.min(sx, sy)
		s = math.max(s, isTouch and 0.58 or 0.38)

		local w  = math.floor(cfg.WindowWidth  * s)
		local h  = math.floor(cfg.WindowHeight * s)
		local sw = math.max(isTouch and 112 or 120, math.floor(cfg.SidebarWidth * s))
	...
	local function doScale()
		...
		if self._sidebar then
			...
			if self._content then
				self._content.Position = UDim2.new(0, sw, 0, 0)
				self._content.Size = UDim2.new(1, -sw, 1, 0)
			end
	...
	self:_initPages()
	self._pageIdx = -1
	self:SetPage(self.cfg.InitialPage or 1)
	task.defer(doScale)
...
function Lib:_buildTitleBar(win)
	...
	new("TextLabel",{Text=cfg.AppName,Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.White,
		BackgroundTransparency=1,Size=UDim2.new(1,-150,0,26),AutomaticSize=Enum.AutomaticSize.None,
		TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=11,LayoutOrder=1},left)

	local verText = "v"..cfg.AppVersion
	local verW = math.clamp((#verText * 7.5) + 28, 64, 150)
	local ver = new("Frame",{Size=UDim2.fromOffset(verW,24),AutomaticSize=Enum.AutomaticSize.None,
		BackgroundColor3=C.Card3,BorderSizePixel=0,ZIndex=11,LayoutOrder=2},left)
	corner(ver,7)
	local verStroke = stroke(ver,fromHex("303030"),1,.25)
	local verInner = new("Frame",{AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromScale(.5,.5),Size=UDim2.new(0,0,1,0),AutomaticSize=Enum.AutomaticSize.X,BackgroundTransparency=1,ZIndex=12},ver)
	hlist(verInner,5)
	local verDot = new("Frame",{Size=UDim2.fromOffset(5,5),BackgroundColor3=accentOrWhite(self),BorderSizePixel=0,ZIndex=12,LayoutOrder=0},verInner)
	corner(verDot,5)
	local verLbl = new("TextLabel",{Text=verText,Font=Enum.Font.GothamBold,TextSize=11,TextColor3=C.TextDim,
		BackgroundTransparency=1,Size=UDim2.fromOffset(math.max(42,#verText*7.5),24),AutomaticSize=Enum.AutomaticSize.None,
		TextXAlignment=Enum.TextXAlignment.Center,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=12,LayoutOrder=1},verInner)
	self._verBadgeInner = verInner
	self._verBadgeLabel = verLbl
	self._verBadgeStroke = verStroke
	self._verBadgeDot = verDot
	self._verBadge = ver
...
function Lib:_buildBody(win)
	...
	local bodyBg = new("Frame",{
		Position=UDim2.fromOffset(0,44),Size=UDim2.new(1,0,1,-44),
		BackgroundColor3=C.Bg2,BorderSizePixel=0,ZIndex=1,
	},win)
	corner(bodyBg,12)
	new("Frame",{Size=UDim2.new(1,0,0,12),BackgroundColor3=C.Bg2,BorderSizePixel=0,ZIndex=1},bodyBg)

	local body = new("Frame",{Position=UDim2.fromOffset(2,44),Size=UDim2.new(1,-4,1,-48),BackgroundTransparency=1,ZIndex=2},win)
	self._body = body
	...
	local ss = new("ScrollingFrame",{Size=UDim2.new(1,0,1,-76),BackgroundTransparency=1,
	...
	self._bar = bar
	self._barAnimGen = 0
	table.insert(self._conns, ss:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
		if self._barTarget and self._bar and self._bar.Visible then
			self:_syncBar()
		end
	end))
	...
	do
		...
		local userCard = new("Frame",{
			AnchorPoint=Vector2.new(.5,1),Position=UDim2.new(.5,0,1,-14),
			Size=UDim2.new(1,-22,0,50),BackgroundColor3=C.Card,
			BorderSizePixel=0,ZIndex=10,ClipsDescendants=true,
		},sidebar)
		corner(userCard,12)
		local avatarRing = new("Frame",{
			AnchorPoint=Vector2.new(0,.5),Position=UDim2.new(0,10,.5,0),
			Size=UDim2.fromOffset(34,34),BackgroundColor3=offline,
			BorderSizePixel=0,ZIndex=11,
		},userCard)
		corner(avatarRing,17)
		local avatar = new("ImageLabel",{
			AnchorPoint=Vector2.new(.5,.5),Position=UDim2.fromScale(.5,.5),
			Size=UDim2.fromOffset(30,30),BackgroundColor3=C.Card2,
			BorderSizePixel=0,Image="",ScaleType=Enum.ScaleType.Crop,ZIndex=12,
		},avatarRing)
		corner(avatar,15)
		local nameLbl = new("TextLabel",{
			Text=LocalPlayer and LocalPlayer.DisplayName or "Player",
			Font=Enum.Font.GothamBold,TextSize=12,TextColor3=C.White,
			BackgroundTransparency=1,Position=UDim2.fromOffset(52,9),
			Size=UDim2.new(1,-82,0,16),TextXAlignment=Enum.TextXAlignment.Left,
			TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=11,
		},userCard)
		local nickLbl = new("TextLabel",{
			Text=LocalPlayer and ("@" .. LocalPlayer.Name) or "@player",
			Font=Enum.Font.Gotham,TextSize=10,TextColor3=C.TextOff,
			BackgroundTransparency=1,Position=UDim2.fromOffset(52,27),
			Size=UDim2.new(1,-82,0,14),TextXAlignment=Enum.TextXAlignment.Left,
			TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=11,
		},userCard)
		local statusDot = new("Frame",{
			AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-14,.5,0),
			Size=UDim2.fromOffset(10,10),BackgroundColor3=offline,
			BorderSizePixel=0,ZIndex=11,
		},userCard)
		corner(statusDot,5)
		pcall(function()
		...
		self._sidebarUserCard = userCard
		self._sidebarUserName = nameLbl
		self._sidebarUserNick = nickLbl
		self._sidebarUserAvatarRing = avatarRing
		self._sidebarUserDot = statusDot
		self:SetSidebarUserStatus(false)
	end
...
function Lib:SetPage(index)
	...
	if newFrame then
		...
		task.delay(fadeDelay, function()
			...
			if gen == self._pageGen then
				...
				if self._tryPlayUpdateLogTitle then
					task.delay(.08, function()
						if gen == self._pageGen then self:_tryPlayUpdateLogTitle() end
					end)
				end
			end
		end)
	...
	if nb then
		...
		self._barTarget = nb.Frame
		self:_animBar(nb.Frame)
	else
		self._barTarget = nil
		self._barAnimGen = (self._barAnimGen or 0) + 1
		if self._bar then
			tw(self._bar, cfg.TweenSpeed or .18, {Size=UDim2.fromOffset(3,0)}, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
			task.delay((cfg.TweenSpeed or .18) + .02, function()
				if not self._barTarget and self._bar then self._bar.Visible = false end
			end)
		end
	end
...
function Lib:_tryPlayUpdateLogTitle()
	if self._bootSplash then return end
	if self._updateLogPageIndex and self._pageIdx == self._updateLogPageIndex and self._playUpdateLogTitle then
		self._playUpdateLogTitle()
	end
end
...
function Lib:_syncBar()
	if not self:_isAlive() then return end
	local bar = self._bar
	local target = self._barTarget
	if not bar or not target or not target.Parent then return end
	local ok, relY = pcall(function()
		local ap = target.AbsolutePosition
		local as = target.AbsoluteSize
		local sp = self._sidebar.AbsolutePosition
...
function Lib:_setCollapsed(collapsed)
	...
	if self._sidebarUserCard then
		if collapsed then
			self._sidebarUserCard.Size = UDim2.new(1,-18,0,46)
			if self._sidebarUserAvatarRing then
				self._sidebarUserAvatarRing.Position = UDim2.new(.5,0,.5,0)
				self._sidebarUserAvatarRing.AnchorPoint = Vector2.new(.5,.5)
			end
			if self._sidebarUserName then self._sidebarUserName.Visible = false end
			if self._sidebarUserNick then self._sidebarUserNick.Visible = false end
			if self._sidebarUserDot then self._sidebarUserDot.Visible = false end
		else
			self._sidebarUserCard.Size = UDim2.new(1,-22,0,50)
			if self._sidebarUserAvatarRing then
				self._sidebarUserAvatarRing.AnchorPoint = Vector2.new(0,.5)
				self._sidebarUserAvatarRing.Position = UDim2.new(0,10,.5,0)
			end
			if self._sidebarUserName then self._sidebarUserName.Visible = true end
			if self._sidebarUserNick then self._sidebarUserNick.Visible = true end
			if self._sidebarUserDot then self._sidebarUserDot.Visible = true end
		end
	end
end

function Lib:SetSidebarUserStatus(running)
	self._sidebarUserRunning = running == true
	local col = self._sidebarUserRunning and C.Green or C.Red
	if self._sidebarUserName and LocalPlayer then
		self._sidebarUserName.Text = LocalPlayer.DisplayName or LocalPlayer.Name or "Player"
	end
	if self._sidebarUserNick and LocalPlayer then
		self._sidebarUserNick.Text = "@" .. tostring(LocalPlayer.Name or "player")
	end
	if self._sidebarUserAvatarRing then
		tw(self._sidebarUserAvatarRing,.18,{BackgroundColor3=col})
	end
	if self._sidebarUserDot then
		tw(self._sidebarUserDot,.18,{BackgroundColor3=col})
	end
end
...
function Lib:Confirm(title, message, onConfirm, onCancel, opts)
	...
	local mw  = math.clamp(math.floor(vp.X * 0.30), 300, 420)
	...
	corner(modal,12)
	local modalStroke = stroke(modal,C.White,1,.72)
	...
	local header=new("Frame",{Size=UDim2.new(1,0,0,62),BackgroundTransparency=1,ZIndex=902,LayoutOrder=0},modal)
	pad(header,14,8,16,16)
	hlist(header,10)
	local iconWrap=new("Frame",{Size=UDim2.fromOffset(26,26),BackgroundTransparency=1,LayoutOrder=0},header)
	...
	local iconLbl=new("TextLabel",{Text=isDestructive and "!" or "i",Font=Enum.Font.GothamBold,TextSize=15,TextColor3=C.Bg,
	...
	new("TextLabel",{Text=title or "Confirm",Font=Enum.Font.GothamBold,TextSize=15,TextColor3=C.White,
		...
		TextTruncate=Enum.TextTruncate.AtEnd},titleCol)
	new("TextLabel",{Text=isDestructive and "This action may affect your session" or "Please review and confirm the action",Font=Enum.Font.Gotham,TextSize=11,TextColor3=C.TextDim,
		...
		TextTruncate=Enum.TextTruncate.AtEnd},titleCol)
	new("Frame",{Position=UDim2.fromOffset(0,61),Size=UDim2.new(1,0,0,1),BackgroundColor3=C.Border2,BorderSizePixel=0,ZIndex=902},modal)
	...
	new("Frame",{Size=UDim2.new(1,0,0,12),BackgroundTransparency=1},body)
	new("Frame",{Position=UDim2.new(0,0,1,0),Size=UDim2.new(1,0,0,1),BackgroundColor3=C.Border2,BorderSizePixel=0},body)

	local footer=new("Frame",{Size=UDim2.new(1,0,0,58),BackgroundTransparency=1,ZIndex=903,LayoutOrder=2},modal)
	...
	local cancelBtn=new("TextButton",{Text=cancelText,Font=Enum.Font.GothamBold,TextSize=13,
		...
		Size=UDim2.new(.5,-5,0,40),AutoButtonColor=false,LayoutOrder=0},footer)
	corner(cancelBtn,8)
	local cancelStroke = stroke(cancelBtn,C.Border2,1,.12)
	cancelBtn.MouseEnter:Connect(function()
		tw(cancelBtn,.1,{TextColor3=C.White,BackgroundColor3=C.Card3})
		tw(cancelStroke,.1,{Color=C.Border3,Transparency=.03})
	end)
	cancelBtn.MouseLeave:Connect(function()
		tw(cancelBtn,.12,{TextColor3=C.TextDim,BackgroundColor3=C.Card2,Size=UDim2.new(.5,-5,0,40)})
		tw(cancelStroke,.12,{Color=C.Border2,Transparency=.12,Thickness=1})
	end)
	cancelBtn.MouseButton1Down:Connect(function()
		tw(cancelBtn,.06,{Size=UDim2.new(.5,-7,0,38)})
		tw(cancelStroke,.06,{Thickness=2})
	end)
	cancelBtn.MouseButton1Up:Connect(function()
		tw(cancelBtn,.16,{Size=UDim2.new(.5,-5,0,40)},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
		tw(cancelStroke,.14,{Thickness=1})
	end)

	local confirmBtn=new("TextButton",{Text=confirmText,Font=Enum.Font.GothamBold,TextSize=13,
		TextColor3=isDestructive and C.White or C.Bg,BackgroundColor3=confirmColor,BorderSizePixel=0,
		Size=UDim2.new(.5,-5,0,40),AutoButtonColor=false,LayoutOrder=1},footer)
	corner(confirmBtn,8)
	local function lighten(col)
		return Color3.new(math.min(col.R+0.15,1),math.min(col.G+0.08,1),math.min(col.B+0.08,1))
	end
	local confirmStroke = stroke(confirmBtn,lighten(confirmColor),1,.1)
	confirmBtn.MouseEnter:Connect(function()
		tw(confirmBtn,.1,{BackgroundColor3=lighten(confirmColor)})
		tw(confirmStroke,.1,{Transparency=0})
	end)
	confirmBtn.MouseLeave:Connect(function()
		tw(confirmBtn,.12,{BackgroundColor3=confirmColor,Size=UDim2.new(.5,-5,0,40)})
		tw(confirmStroke,.12,{Transparency=.1,Thickness=1})
	end)
	confirmBtn.MouseButton1Down:Connect(function()
		tw(confirmBtn,.06,{Size=UDim2.new(.5,-7,0,38)})
		tw(confirmStroke,.06,{Thickness=2})
	end)
	confirmBtn.MouseButton1Up:Connect(function()
		tw(confirmBtn,.16,{Size=UDim2.new(.5,-5,0,40)},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
		tw(confirmStroke,.14,{Thickness=1})
	end)
	...
	tw(modalStroke,.3,{Transparency=.55},Enum.EasingStyle.Quint)
end
...
function Lib:_revealWindowAfterSplash()
	local tw2, th2
	if self._computeScale then
	...
	self.Window.Visible = true
	self.Window.BackgroundTransparency = 1
	self.Window.Size = UDim2.fromOffset(math.floor(tw2 * 0.92), math.floor(th2 * 0.92))
	self.Window.Position = UDim2.new(.5, 0, .5, 14)
	tw(self.Window, .45, {
		BackgroundTransparency = 0,
		Size = UDim2.fromOffset(tw2, th2),
		Position = UDim2.fromScale(.5, .5),
	}, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
	task.delay(.5, function()
		...
		if self._tryPlayUpdateLogTitle then
			self:_tryPlayUpdateLogTitle()
		end
	end)
end

function Lib:_beginBootSplash()
	if self._bootSplash then return end
	local cfg = self.cfg
	self.Window.Visible = false
	self._bootFinished = false
	self._bootProgress = 0
	self._bootVisualProgress = 0
	self._bootTargetProgress = 0
	self._bootStepTotal = math.max(1, tonumber(cfg.BootSteps) or #(cfg.SplashTasks or {}) or 1)
	self._bootLineCount = 0
	self._bootLastNorm = nil

	local overlay = new("Frame",{
		Size=UDim2.fromScale(1,1),BackgroundColor3=fromHex("000000"),
		BackgroundTransparency=1,BorderSizePixel=0,ZIndex=700,
	},self._sg)
	local card = new("Frame",{
		AnchorPoint=Vector2.new(.5,.5),Position=UDim2.new(.5,0,.52,0),
		Size=UDim2.fromOffset(430,286),BackgroundColor3=fromHex("090909"),
		BorderSizePixel=0,ZIndex=701,BackgroundTransparency=1,
		ClipsDescendants=true,
	},overlay)
	corner(card,12)
	local cardStroke = stroke(card,C.White,1,.72)
	new("UIGradient",{
		Color=ColorSequence.new{
			ColorSequenceKeypoint.new(0, fromHex("101010")),
			ColorSequenceKeypoint.new(1, fromHex("050505")),
		},
		Rotation=90,
	},card)

	local accent = new("Frame",{Position=UDim2.fromOffset(12,20),Size=UDim2.fromOffset(3,58),
		BackgroundColor3=accentOrWhite(self),BorderSizePixel=0,ZIndex=702},card)
	corner(accent,2)
	local top = new("Frame",{Position=UDim2.fromOffset(28,20),Size=UDim2.new(1,-50,0,58),
		BackgroundTransparency=1,ZIndex=702},card)
	local logo = new("Frame",{Size=UDim2.fromOffset(42,42),BackgroundTransparency=1,
		BorderSizePixel=0,ZIndex=703},top)
	if cfg.LogoImage ~= "" then
		new("ImageLabel",{Size=UDim2.fromScale(1,1),BackgroundTransparency=1,Image=cfg.LogoImage,
			ScaleType=Enum.ScaleType.Fit,ZIndex=704},logo)
	else
		new("TextLabel",{Size=UDim2.fromScale(1,1),BackgroundTransparency=1,
			Text=string.upper(string.sub(cfg.AppName,1,1)),Font=Enum.Font.GothamBold,
			TextSize=19,TextColor3=C.White,ZIndex=704},logo)
	end
	local title = new("TextLabel",{Text=cfg.AppName,Font=Enum.Font.GothamBold,TextSize=18,
		TextColor3=C.White,BackgroundTransparency=1,Position=UDim2.fromOffset(54,1),
		Size=UDim2.new(1,-150,0,24),TextXAlignment=Enum.TextXAlignment.Left,
		TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=703},top)
	local sub = new("TextLabel",{Text=cfg.AppSubtitle,Font=Enum.Font.Gotham,TextSize=11,
		TextColor3=C.TextDim,BackgroundTransparency=1,Position=UDim2.fromOffset(54,28),
		Size=UDim2.new(1,-150,0,16),TextXAlignment=Enum.TextXAlignment.Left,
		TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=703},top)
	local ver = new("TextLabel",{Text="v" .. tostring(cfg.AppVersion or "1.0"),Font=Enum.Font.GothamBold,
		TextSize=10,TextColor3=C.TextDim,BackgroundColor3=C.Card2,BorderSizePixel=0,
		AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,0,0,5),Size=UDim2.fromOffset(74,24),
		TextXAlignment=Enum.TextXAlignment.Center,ZIndex=703},top)
	corner(ver,8)
	stroke(ver,C.Border2,1,.2)

	local section = new("TextLabel",{Text="BOOT SEQUENCE",Font=Enum.Font.GothamBold,TextSize=9,
		TextColor3=C.TextOff,BackgroundTransparency=1,Position=UDim2.fromOffset(22,90),
		Size=UDim2.new(1,-44,0,14),TextXAlignment=Enum.TextXAlignment.Left,ZIndex=702},card)
	local status = new("TextLabel",{Text="Preparing runtime",Font=Enum.Font.GothamBold,TextSize=14,
		TextColor3=C.White,BackgroundTransparency=1,Position=UDim2.fromOffset(22,112),
		Size=UDim2.new(1,-88,0,20),TextXAlignment=Enum.TextXAlignment.Left,
		TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=702},card)
	local pct = new("TextLabel",{Text="0%",Font=Enum.Font.Code,TextSize=13,TextColor3=C.TextDim,
		BackgroundTransparency=1,AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-22,0,112),
		Size=UDim2.fromOffset(54,20),TextXAlignment=Enum.TextXAlignment.Right,ZIndex=702},card)

	local barBg = new("Frame",{Position=UDim2.fromOffset(22,146),Size=UDim2.new(1,-44,0,8),
		BackgroundColor3=C.Card3,BorderSizePixel=0,ZIndex=702},card)
	corner(barBg,4)
	local barFill = new("Frame",{Size=UDim2.new(0,0,1,0),BackgroundColor3=accentOrWhite(self),
		BorderSizePixel=0,ZIndex=703},barBg)
	corner(barFill,4)

	local logPanel = new("Frame",{Position=UDim2.fromOffset(22,174),Size=UDim2.new(1,-44,0,68),
		BackgroundColor3=fromHex("030303"),BorderSizePixel=0,ZIndex=702,ClipsDescendants=true},card)
	corner(logPanel,8)
	stroke(logPanel,fromHex("2b2b2b"),1,.1)
	local logHolder = new("Frame",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,ZIndex=703},logPanel)
	pad(logHolder,8,8,12,12)
	vlist(logHolder,2)

	local readyDot = new("Frame",{AnchorPoint=Vector2.new(0,1),Position=UDim2.new(0,22,1,-18),
		Size=UDim2.fromOffset(7,7),BackgroundColor3=C.Yellow,BorderSizePixel=0,ZIndex=702},card)
	corner(readyDot,4)
	local hint = new("TextLabel",{Text="Loading real script modules",Font=Enum.Font.Code,TextSize=10,
		TextColor3=C.TextOff,BackgroundTransparency=1,Position=UDim2.new(0,36,1,-25),
		Size=UDim2.new(1,-58,0,16),TextXAlignment=Enum.TextXAlignment.Left,ZIndex=702},card)

	self._bootSplash = {
		Overlay=overlay, Card=card, Stroke=cardStroke, Accent=accent,
		Status=status, Percent=pct, Fill=barFill, LogHolder=logHolder,
		Dot=readyDot, Hint=hint,
	}
	if self._bootFillConn then pcall(function() self._bootFillConn:Disconnect() end) end
	self._bootFillConn = RunService.Heartbeat:Connect(function(dt)
		local boot = self._bootSplash
		if not boot then return end
		local target = math.clamp(self._bootTargetProgress or self._bootProgress or 0, 0, 1)
		local current = math.clamp(self._bootVisualProgress or 0, 0, 1)
		local alpha = 1 - math.exp(-math.max(dt or 0, 0) * 9)
		current = current + (target - current) * alpha
		if math.abs(target - current) < 0.002 then current = target end
		self._bootVisualProgress = current
		if boot.Fill then boot.Fill.Size = UDim2.fromScale(current, 1) end
	end)
	tw(overlay,.25,{BackgroundTransparency=.28},Enum.EasingStyle.Quint)
	tw(card,.42,{BackgroundTransparency=0,Position=UDim2.fromScale(.5,.5)},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
	tw(cardStroke,.35,{Transparency=.55},Enum.EasingStyle.Quint)
	task.spawn(function()
		while self._bootSplash and not self._bootFinished do
			tw(readyDot,.35,{BackgroundTransparency=.45})
			task.wait(.35)
			tw(readyDot,.35,{BackgroundTransparency=0})
			task.wait(.35)
		end
	end)
	self:SetBootStep((cfg.SplashTasks and cfg.SplashTasks[1]) or "Preparing runtime", .04, "runtime")
end

function Lib:SetBootStep(text, progress, detail)
	local boot = self._bootSplash
	if not boot then return end
	text = tostring(text or "Loading")
	local normalized = text:lower():gsub("[%p%s]+","")
	local isDuplicate = normalized ~= "" and normalized == self._bootLastNorm
	if progress == nil then
		self._bootProgress = math.min(0.96, (self._bootProgress or 0) + (1 / math.max(self._bootStepTotal, 1)))
	else
		self._bootProgress = math.clamp(tonumber(progress) or 0, 0, 1)
	end
	self._bootLastNorm = normalized
	self._bootTargetProgress = self._bootProgress
	local pct = math.floor((self._bootProgress or 0) * 100 + .5)
	if boot.Status then boot.Status.Text = text end
	if boot.Percent then boot.Percent.Text = tostring(pct) .. "%" end
	if boot.Hint then boot.Hint.Text = tostring(detail or "loading") end
	if boot.LogHolder and not isDuplicate then
		self._bootLineCount = (self._bootLineCount or 0) + 1
		local row = new("TextLabel",{Text="> " .. text,Font=Enum.Font.Code,TextSize=10,
			TextColor3=C.TextDim,BackgroundTransparency=1,Size=UDim2.new(1,0,0,12),
			TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,
			LayoutOrder=self._bootLineCount,ZIndex=704},boot.LogHolder)
		local children = {}
		for _, child in ipairs(boot.LogHolder:GetChildren()) do
			if child:IsA("TextLabel") then children[#children+1] = child end
		end
		table.sort(children,function(a,b) return a.LayoutOrder < b.LayoutOrder end)
		while #children > 4 do
			local first = table.remove(children,1)
			if first then first:Destroy() end
		end
		tw(row,.16,{TextColor3=C.White})
	end
end

function Lib:FinishBootSplash(finalText, targetPage)
	if self._bootFinished then return end
	local boot = self._bootSplash
	if not boot then
		self._bootFinished = true
		if targetPage then pcall(function() self:SetPage(targetPage) end) end
		self:_revealWindowAfterSplash()
		return
	end
	self:SetBootStep(finalText or "Ready", 1, "interface ready")
	self._bootFinished = true
	if boot.Dot then tw(boot.Dot,.18,{BackgroundColor3=C.Green,BackgroundTransparency=0}) end
	task.delay(.28,function()
		if targetPage then pcall(function() self:SetPage(targetPage) end) end
		if boot.Overlay then tw(boot.Overlay,.28,{BackgroundTransparency=1},Enum.EasingStyle.Quint) end
		if boot.Card then tw(boot.Card,.3,{BackgroundTransparency=1,Position=UDim2.new(.5,0,.47,0)},Enum.EasingStyle.Quint,Enum.EasingDirection.In) end
		task.delay(.32,function()
			if boot.Overlay and boot.Overlay.Parent then boot.Overlay:Destroy() end
			self._bootSplash = nil
			if self._bootFillConn then
				pcall(function() self._bootFillConn:Disconnect() end)
				self._bootFillConn = nil
			end
			self:_revealWindowAfterSplash()
		end)
	end)
...
function Lib:AddSectionHeader(pi,title,sub)
	...
	self:_gap(s,pi,4)
	local hasSub = sub ~= nil and tostring(sub) ~= ""
	local headerH = hasSub and 48 or 32
	local header = new("Frame",{
		Size=UDim2.new(1,0,0,headerH),BackgroundTransparency=1,
		LayoutOrder=self:_o(pi),
	},s)
	local marker = new("Frame",{
		Size=UDim2.new(0,3,1,hasSub and -8 or -10),
	...
	if title then
		new("TextLabel",{Text=title,Font=Enum.Font.GothamBold,TextSize=20,TextColor3=C.White,
			BackgroundTransparency=1,Position=UDim2.fromOffset(14,0),Size=UDim2.new(1,-14,0,28),
	...
	if hasSub then
		new("TextLabel",{Text=sub,Font=Enum.Font.Gotham,TextSize=12,TextColor3=C.TextDim,
			BackgroundTransparency=1,Position=UDim2.fromOffset(14,28),Size=UDim2.new(1,-14,0,18),
			TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},header)
	end
...
function Lib:AddButtonRow(pi,defs)
	...
	for _,def in ipairs(defs) do
		local textLen = #tostring((def or {}).Text or "")
		if textLen > 18 then
			rowH = math.max(rowH, math.min(68, 34 + math.ceil(textLen / 18) * 12))
		end
	end
	local row=new("Frame",{Size=UDim2.new(1,0,0,rowH),BackgroundTransparency=1,LayoutOrder=self:_o(pi)},s)
	row:SetAttribute("ComponentId", self:_nextCompId("ButtonRow"))
	hlist(row,10)

	local styles={
		primary = {bg=C.White,    tc=C.Bg,    hov=fromHex("ededed"),dn=fromHex("cfcfcf"), sc=fromHex("ffffff"), hsc=fromHex("f6f6f6")},
		danger  = {bg=C.RedBg,    tc=C.Red,   hov=fromHex("180707"),dn=fromHex("240808"), sc=fromHex("6f2222"), hsc=C.Red},
		warning = {bg=C.YellowBg, tc=C.Yellow,hov=fromHex("171202"),dn=fromHex("221a04"), sc=fromHex("6b5414"), hsc=C.Yellow},
		ghost   = {bg=C.Card2,    tc=C.Text,  hov=C.Card3,          dn=C.Card,           sc=C.Border2, hsc=C.Border3},
		outline = {bg=C.Card,     tc=C.Text,  hov=C.Card2,          dn=C.Sidebar,        sc=C.Border,  hsc=C.Border3},
		success = {bg=C.GreenBg,  tc=C.Green, hov=fromHex("061a0d"),dn=fromHex("082612"), sc=fromHex("145d34"), hsc=C.Green},
		purple  = {bg=C.PurpleBg, tc=C.Purple,hov=fromHex("150820"),dn=fromHex("200b32"), sc=fromHex("5e2884"), hsc=C.Purple},
	}
	...
	for i,def in ipairs(defs) do
		...
		local btn=new("TextButton",{Text=def.Text or "",Font=Enum.Font.GothamBold,TextSize=12,
			...
			Size=UDim2.fromOffset(w,rowH-4),AutoButtonColor=false,LayoutOrder=i,
			TextWrapped=true},row)
		corner(btn,8)
		local btnStroke = stroke(btn,st.sc or C.Border2,1,def.Style=="primary" and .35 or .15)
		btn.MouseEnter:Connect(function()
			tw(btn,.12,{BackgroundColor3=st.hov})
			tw(btnStroke,.12,{Color=st.hsc or st.sc or C.Border3,Transparency=def.Style=="primary" and .2 or .05})
		end)
		btn.MouseLeave:Connect(function()
			tw(btn,.15,{BackgroundColor3=st.bg})
			tw(btnStroke,.15,{Color=st.sc or C.Border2,Transparency=def.Style=="primary" and .35 or .15,Thickness=1})
		end)
		btn.MouseButton1Down:Connect(function()
			tw(btn,.07,{BackgroundColor3=st.dn,Size=UDim2.fromOffset(w-4,rowH-6)})
			tw(btnStroke,.07,{Thickness=2})
		end)
		btn.MouseButton1Up:Connect(function()
			tw(btn,.2,{BackgroundColor3=st.hov,Size=UDim2.fromOffset(w,rowH-4)},Enum.EasingStyle.Back,Enum.EasingDirection.Out)
			tw(btnStroke,.18,{Thickness=1})
		end)
...
function Lib:AddToggle(pi,label,default,callback,desc)
	local s=self:GetPage(pi); if not s then return end
	local labelLines = math.min(2, math.max(1, math.ceil(#tostring(label or "") / 52)))
	local descLen = #tostring(desc or "")
	local descLines = desc and math.min(3, math.max(1, math.ceil(descLen / 90))) or 0
	local rowH = desc and math.max(54, 30 + labelLines * 15 + descLines * 11) or math.max(48, 30 + labelLines * 15)
	local descY = desc and (descLines <= 1 and (16 + labelLines * 15) or (12 + labelLines * 15)) or 0
	...
	if desc then
		new("TextLabel",{Text=label or "",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.Text,
			BackgroundTransparency=1,Position=UDim2.new(0,0,0,8),Size=UDim2.new(1,-82,0,labelLines * 15),
		...
		new("TextLabel",{Text=desc,Font=Enum.Font.Gotham,TextSize=10,TextColor3=C.TextDim,
			BackgroundTransparency=1,Position=UDim2.new(0,0,0,descY),Size=UDim2.new(1,-82,0,rowH - descY - 6),
		...
		new("TextLabel",{Text=label or "",Font=Enum.Font.Gotham,TextSize=13,TextColor3=C.Text,
			BackgroundTransparency=1,Size=UDim2.new(1,-82,1,0),TextXAlignment=Enum.TextXAlignment.Left,
			TextWrapped=true},row)
	end
...
function Lib:AddInputNumber(pi, labelTxt, opts, callback)
	...
	local allowedText = opts.AllowedText or opts.AllowText
	local s=self:GetPage(pi); if not s then return end
	if labelTxt then
		new("TextLabel",{Text=labelTxt,Font=Enum.Font.GothamBold,TextSize=11,TextColor3=C.TextDim,
			BackgroundTransparency=1,Size=UDim2.new(1,0,0,18),
	...
	box.FocusLost:Connect(function(enter)
		tw(wrap,.15,{BackgroundColor3=C.Card}); tw(ws,.18,{Color=C.Border})
		local cleanText = tostring(box.Text or ""):gsub("^%s*(.-)%s*$","%1")
		if allowedText then
			local allowed = allowedText == true
			if type(allowedText) == "table" then
				allowed = allowedText[cleanText] == true or allowedText[cleanText:lower()] == true
			end
			if allowed then
				errLbl.Text=""
				if callback then self:_safeCall(callback, cleanText, enter or UserInputService.TouchEnabled) end
				return
			end
		end
...
function Lib:AddMultiSelect(pi, labelTxt, options, callback)
	...
	local wrapper=new("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
		BackgroundColor3=C.Card,BorderSizePixel=0,LayoutOrder=self:_o(pi),ClipsDescendants=true},s)
	...
	corner(wrapper,10)
	local wrapperStroke = stroke(wrapper,C.Border,1)
	...
	local header=new("Frame",{Size=UDim2.new(1,0,0,52),BackgroundTransparency=1,LayoutOrder=0},wrapper)
	pad(header,0,0,16,16)
	new("TextLabel",{Text=string.upper(labelTxt or "Select"),Font=Enum.Font.GothamBold,TextSize=9,TextColor3=C.TextOff,
		BackgroundTransparency=1,Position=UDim2.fromOffset(0,7),Size=UDim2.new(1,-34,0,12),
		TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd},header)

	local countLbl=new("TextLabel",{Text="none",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.Text,
		BackgroundTransparency=1,Position=UDim2.fromOffset(0,22),
		Size=UDim2.new(1,-48,0,20),TextXAlignment=Enum.TextXAlignment.Left,
		TextYAlignment=Enum.TextYAlignment.Center,TextTruncate=Enum.TextTruncate.AtEnd},header)
	...
	local function refreshCount(fireCallback)
		local labels={}
	...
	for i,opt in ipairs(options) do
		...
		local hv=new("Frame",{BackgroundColor3=C.Card2,BackgroundTransparency=1,
			Size=UDim2.new(1,-12,1,-6),Position=UDim2.fromOffset(6,3),
		...
		local optLbl=new("TextLabel",{Text=tostring(opt),Font=Enum.Font.Gotham,TextSize=13,TextColor3=C.Text,
			...
			Size=UDim2.new(1,-48,1,0),TextXAlignment=Enum.TextXAlignment.Left,
			TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=53},row)
	...
	toggleBtn.Activated:Connect(function()
		...
		if open then
			...
			tw(wrapperStroke,.16,{Color=accentOrWhite(self)})
			...
			tw(wrapperStroke,.18,{Color=C.Border})
	...
	header.MouseEnter:Connect(function() tw(wrapperStroke,.15,{Color=C.Border3}) end)
	...
	function obj:SetSelected(tbl, fireCallback)
		...
		if type(tbl)=="table" then
			for _,opt in ipairs(tbl) do
				if selected[opt] ~= nil then selected[opt]=true end
			end
		...
		refreshCount(fireCallback == true)
	end
...
function Lib:AddActionList(pi, labelTxt, opts)
	opts = opts or {}
	local s=self:GetPage(pi); if not s then return end
	local items = {}
	local itemH = opts.ItemHeight or 44
	local emptyText = opts.EmptyText or "No items configured"
	local maxItems = opts.MaxItems or 50
	...
	corner(wrap,10)
	stroke(wrap,C.Border,1)
	new("UIListLayout",{FillDirection=Enum.FillDirection.Vertical,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,0)},wrap)

	if labelTxt then
		local hdr=new("Frame",{Size=UDim2.new(1,0,0,34),BackgroundColor3=C.Card2,BorderSizePixel=0,LayoutOrder=0},wrap)
		corner(hdr,10)
		new("Frame",{Position=UDim2.new(0,0,1,-8),Size=UDim2.new(1,0,0,8),BackgroundColor3=C.Card2,BorderSizePixel=0},hdr)
		pad(hdr,0,0,16,16)
		new("TextLabel",{Text=string.upper(labelTxt),Font=Enum.Font.GothamBold,TextSize=10,TextColor3=C.TextDim,
			BackgroundTransparency=1,Size=UDim2.fromScale(1,1),TextXAlignment=Enum.TextXAlignment.Left},hdr)
	end
	...
	new("UIListLayout",{SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,8)},listFrame)

	local emptyRow=new("TextLabel",{Text=emptyText,Font=Enum.Font.Gotham,TextSize=12,TextColor3=C.TextOff,
		BackgroundTransparency=1,Size=UDim2.new(1,0,0,28),TextXAlignment=Enum.TextXAlignment.Left,
		LayoutOrder=0},listFrame)

	local function refreshEmpty()
		emptyRow.Visible = #items == 0
	end

	local function makeIconButton(parent, action, order)
		action = action or {}
		local isDanger = action.Style == "danger"
		local isSuccess = action.Style == "success"
		local bg = isDanger and C.RedBg or (isSuccess and C.GreenBg or C.Card3)
		local hov = isDanger and fromHex("210808") or (isSuccess and fromHex("071b0d") or C.Card2)
		local sc = isDanger and fromHex("6f2222") or (isSuccess and fromHex("145d34") or C.Border2)
		local btn=new("ImageButton",{Image="rbxassetid://" .. tostring(action.Icon or ""),
			ImageColor3=action.ImageColor or C.White,BackgroundColor3=bg,BorderSizePixel=0,
			Size=UDim2.fromOffset(30,30),LayoutOrder=order,AutoButtonColor=false,
			Active=true,Selectable=false},parent)
		if not btn then return nil end
		corner(btn,8)
		local bs = stroke(btn,sc,1,.15)
		if btn.MouseEnter then
			btn.MouseEnter:Connect(function()
				tw(btn,.12,{BackgroundColor3=hov})
				tw(bs,.12,{Transparency=.04,Color=isDanger and C.Red or (isSuccess and C.Green or C.Border3)})
			end)
		end
		if btn.MouseLeave then
			btn.MouseLeave:Connect(function()
				tw(btn,.14,{BackgroundColor3=bg,Size=UDim2.fromOffset(30,30)})
				tw(bs,.14,{Transparency=.15,Color=sc})
			end)
		end
		if btn.MouseButton1Down then
			btn.MouseButton1Down:Connect(function() tw(btn,.07,{Size=UDim2.fromOffset(28,28)}) end)
		end
		if btn.MouseButton1Up then
			btn.MouseButton1Up:Connect(function() tw(btn,.16,{Size=UDim2.fromOffset(30,30)},Enum.EasingStyle.Back,Enum.EasingDirection.Out) end)
		end
		pcall(function() btn.CursorIcon="rbxasset://SystemCursors/PointingHand" end)
		return btn
	end
	...
	self:_gap(s,pi,10)
	local lib = self
	local obj={Frame=wrap,ListFrame=listFrame}
	function obj:AddItem(id, text, actions)
		if #items >= maxItems then return nil end
		local row=new("Frame",{Size=UDim2.new(1,0,0,itemH),BackgroundColor3=C.Card2,
			BorderSizePixel=0,LayoutOrder=#items+1},listFrame)
		corner(row,8)
		local rs=stroke(row,C.Border2,1,.12)
		pad(row,0,0,10,12)
		local actionHolder=new("Frame",{Size=UDim2.fromOffset(68,itemH),BackgroundTransparency=1,
			LayoutOrder=0},row)
		actionHolder.Position=UDim2.fromOffset(0,0)
		hlist(actionHolder,6,Enum.VerticalAlignment.Center)
		local label=new("TextLabel",{Text=tostring(text or ""),Font=Enum.Font.Gotham,TextSize=12,
			TextColor3=C.Text,BackgroundTransparency=1,Position=UDim2.fromOffset(78,0),
			Size=UDim2.new(1,-82,1,0),TextXAlignment=Enum.TextXAlignment.Left,
			TextTruncate=Enum.TextTruncate.AtEnd},row)
		for i, action in ipairs(validateTable(actions, {})) do
			local btn = makeIconButton(actionHolder, action, i)
			if btn and btn.Activated then
				btn.Activated:Connect(function()
					if lib._debounce and not lib:_debounce("actionlist_"..tostring(id).."_"..tostring(i),0.18) then return end
					if typeof(action.Callback) == "function" then
						task.spawn(function()
							local ok, err = pcall(action.Callback, id, text, obj)
							if not ok then warn("[SlaoqUILib] action list callback failed: " .. tostring(err)) end
						end)
					end
				end)
			end
		...
		row.MouseEnter:Connect(function()
			tw(row,.12,{BackgroundColor3=C.Card3})
			tw(rs,.12,{Color=C.Border3,Transparency=.05})
		end)
		row.MouseLeave:Connect(function()
			tw(row,.16,{BackgroundColor3=C.Card2})
			tw(rs,.16,{Color=C.Border2,Transparency=.12})
		end)
		items[#items+1]={Id=id,Text=text,Row=row,Label=label}
		refreshEmpty()
		return row
	end
	function obj:Clear()
	...
	function obj:RemoveById(id)
		...
		for i, item in ipairs(items) do
			if item.Row then item.Row.LayoutOrder=i end
		end
		refreshEmpty()
	end
	function obj:Count() return #items end
	function obj:GetItems()
		local out={}
		for _, item in ipairs(items) do out[#out+1]=item.Text end
		return out
	end
	refreshEmpty()
	return obj
...
function Lib:AddDropdown(pi,labelTxt,options,callback)
	...
	local open=false
	local wrapper=new("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,
		BackgroundColor3=C.Card,BorderSizePixel=0,ClipsDescendants=true,LayoutOrder=self:_o(pi),ZIndex=50},s)
	...
	corner(wrapper,10)
	local wrapperStroke = stroke(wrapper,C.Border,1)
	new("UIListLayout",{FillDirection=Enum.FillDirection.Vertical,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,0)},wrapper)

	local headerH = labelTxt and 52 or 44
	local header=new("Frame",{Size=UDim2.new(1,0,0,headerH),BackgroundTransparency=1,LayoutOrder=0,ZIndex=51},wrapper)
	pad(header,0,0,16,16)
	if labelTxt then
		new("TextLabel",{Text=string.upper(labelTxt),Font=Enum.Font.GothamBold,TextSize=9,TextColor3=C.TextOff,
			BackgroundTransparency=1,Position=UDim2.fromOffset(0,7),Size=UDim2.new(1,-34,0,12),
			TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=52},header)
	end
	local selLbl=new("TextLabel",{Text=tostring(selected),Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.Text,
		BackgroundTransparency=1,Position=UDim2.fromOffset(0,labelTxt and 22 or 0),
		Size=UDim2.new(1,-48,0,labelTxt and 20 or headerH),TextXAlignment=Enum.TextXAlignment.Left,
		TextYAlignment=Enum.TextYAlignment.Center,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=52},header)
	local arrow=new("TextLabel",{Text="v",Font=Enum.Font.GothamBold,TextSize=11,TextColor3=C.TextDim,
		BackgroundTransparency=1,AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-4,.5,0),
		Size=UDim2.fromOffset(22,22),TextXAlignment=Enum.TextXAlignment.Center,ZIndex=53},header)

	local listH=1 + math.max(#options,1)*40
	local optList=new("Frame",{Size=UDim2.new(1,0,0,0),
		BackgroundColor3=C.Card,BorderSizePixel=0,ClipsDescendants=true,ZIndex=60,Visible=false,LayoutOrder=1},wrapper)
	corner(optList,10)
	new("Frame",{Size=UDim2.new(1,-32,0,1),AnchorPoint=Vector2.new(.5,0),Position=UDim2.new(.5,0,0,0),
		BackgroundColor3=C.Border2,BorderSizePixel=0,ZIndex=61},optList)
	local listInner=new("Frame",{Position=UDim2.fromOffset(0,1),Size=UDim2.new(1,0,0,0),
		AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,BorderSizePixel=0,ZIndex=61},optList)
	vlist(listInner,0)
	wrapper:SetAttribute("AriaRole","listbox")
	wrapper:SetAttribute("AriaLabel", validateString(labelTxt,"Dropdown"))
	...
	local function updateSelectedVisual()
		for i,entry in ipairs(optionRows) do
			local active = isSelectedOpt(i)
			if entry.Hv then entry.Hv.BackgroundTransparency = active and .18 or 1 end
			if entry.Check then entry.Check.BackgroundTransparency = active and 0 or 1 end
			if entry.Label then entry.Label.TextColor3 = active and C.White or C.Text end
		end
	end
	...
	local function renderOptions()
		...
		optionRows = {}
		listH = 1 + math.max(#options,1)*40
		if #options == 0 then
			local empty=new("Frame",{Size=UDim2.new(1,0,0,40),BackgroundTransparency=1,LayoutOrder=1,ZIndex=61},listInner)
			pad(empty,0,0,16,16)
			new("TextLabel",{Text="No options",Font=Enum.Font.Gotham,TextSize=12,TextColor3=C.TextDim,
				BackgroundTransparency=1,Size=UDim2.fromScale(1,1),TextXAlignment=Enum.TextXAlignment.Left,ZIndex=62},empty)
			return
		end
		for i,opt in ipairs(options) do
			local row=new("Frame",{Size=UDim2.new(1,0,0,40),BackgroundTransparency=1,BorderSizePixel=0,LayoutOrder=i,ZIndex=61},listInner)
			local hv=new("Frame",{BackgroundColor3=C.Card2,BackgroundTransparency=isSelectedOpt(i) and .18 or 1,
				Size=UDim2.new(1,-12,1,-6),Position=UDim2.fromOffset(6,3),BorderSizePixel=0,ZIndex=60},row)
			corner(hv,7)
			local optLbl=new("TextLabel",{Text=tostring(opt),Font=Enum.Font.Gotham,TextSize=13,TextColor3=isSelectedOpt(i) and C.White or C.Text,
				BackgroundTransparency=1,Size=UDim2.new(1,-48,1,0),Position=UDim2.fromOffset(16,0),
				TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=61},row)
			local checkDot=new("Frame",{AnchorPoint=Vector2.new(1,.5),Position=UDim2.new(1,-16,.5,0),
				Size=UDim2.fromOffset(8,8),BorderSizePixel=0,ZIndex=62,
				BackgroundColor3=accentOrWhite(self),BackgroundTransparency=isSelectedOpt(i) and 0 or 1},row)
			corner(checkDot,4)
			row:SetAttribute("AriaRole","option")
			row:SetAttribute("AriaLabel", tostring(opt))
			local btnOpt=new("TextButton",{Text="",BackgroundTransparency=1,Size=UDim2.fromScale(1,1),AutoButtonColor=false,ZIndex=62},row)
			btnOpt.MouseEnter:Connect(function() tw(hv,.16,{BackgroundTransparency=isSelectedOpt(i) and .12 or .28}) end)
			btnOpt.MouseLeave:Connect(function()
				tw(hv,.18,{BackgroundTransparency=isSelectedOpt(i) and .18 or 1})
			end)
			btnOpt.Activated:Connect(function()
				...
				tw(wrapper,.15,{BackgroundColor3=C.Card})
				tw(wrapperStroke,.15,{Color=C.Border})
				task.delay(.2,function() if optList then optList.Visible=false end end)
				if callback then self:_safeCall(callback, opt) end
			end)
			optionRows[i] = {Row=row, Hv=hv, Label=optLbl, Check=checkDot}
		end
	end
	renderOptions()

	local toggleBtn=new("TextButton",{Text="",BackgroundTransparency=1,Size=UDim2.fromScale(1,1),ZIndex=55,AutoButtonColor=false},header)
	pcall(function() toggleBtn.CursorIcon="rbxasset://SystemCursors/PointingHand" end)
	header.MouseEnter:Connect(function()
		tw(wrapper,.12,{BackgroundColor3=C.Card2})
		tw(wrapperStroke,.12,{Color=C.Border3})
	end)
	header.MouseLeave:Connect(function()
		if not open then
			tw(wrapper,.15,{BackgroundColor3=C.Card})
	...
	self:_gap(s,pi,8)
	local obj={Button=toggleBtn,List=optList,Frame=wrapper}
	function obj:GetSelected()
		return selected
	end
	...
	function obj:SetOptions(newOptions, newSelected, fireCallback)
		options = validateTable(newOptions, {})
		local nextSelected = newSelected
		local exists = false
		for _, opt in ipairs(options) do
			if opt == nextSelected then exists = true; break end
		end
		if not exists then nextSelected = options[1] or "" end
...
function Lib:_runDemo()
	local demoUpdateLogs = {
		{
			Version = "v3.8-demo",
			Lines = {
				{Text="Biome Script UI update log demo", Bold=true, Color=Color3.fromRGB(255,255,255)},
				{Text="This screen is rendered directly from biomescript_uilibrary.lua, so visual changes can be tested locally.", Color=Color3.fromRGB(125,220,255)},
				"Mention pills: open #Dashboard, inspect #Inputs, or jump to #Logs.",
				{Text="Bold + color sample: #Components and #Buttons should animate like navigation chips.", Bold=true, Color=Color3.fromRGB(190,255,170)},
				{Text="Terminal line wrapping sample: long lines should stay inside the changelog panel and keep a clean monolith look without touching the border.", Color=Color3.fromRGB(255,205,105)},
			},
		},
		{
			Version = "v3.7-demo",
			Lines = {
				{Text="Older update tab", Bold=true, Color=Color3.fromRGB(255,255,255)},
				"This version exists only to test tab switching animations.",
				"Shortcut test: #Dashboard should navigate away from this hidden page.",
			},
		},
	}
	self:ShowUpdateLog(demoUpdateLogs, {
		PageName = "Update Log",
		Title = "Update Log",
		Subtitle = "Local visual test for Biome Script",
	})
	...
	task.defer(function()
		if self:_isAlive() then self:SetPage(6) end
	end)
end
...
function Lib:FindPageIndex(name)
	local target = tostring(name or ""):lower():gsub("%s+", "")
	if target == "" then return nil end
	for i, page in ipairs(self.cfg.Pages or {}) do
		if page.Name and tostring(page.Name):lower():gsub("%s+", "") == target then
			return i
		end
	end
	return nil
...
function Lib:ShowUpdateLog(logs, opts)
	opts = opts or {}
	logs = validateTable(logs, {})
	local pageName = opts.PageName or "Update Log"
	local pageIndex = opts.PageIndex or self:FindPageIndex(pageName)
	local page = pageIndex and self._pages[pageIndex]
	if not page or not page.Scroll then return nil end
	self._updateLogPageIndex = pageIndex
	if self._verBadge and not self._updateLogBadgeButton then
		local badgeButton = new("TextButton",{Text="",BackgroundTransparency=1,Size=UDim2.fromScale(1,1),ZIndex=20,AutoButtonColor=false},self._verBadge)
		self._updateLogBadgeButton = badgeButton
		badgeButton.MouseEnter:Connect(function()
			tw(self._verBadge,.14,{BackgroundColor3=C.Card2})
			if self._verBadgeLabel then tw(self._verBadgeLabel,.14,{TextColor3=C.White}) end
			if self._verBadgeStroke then tw(self._verBadgeStroke,.14,{Color=accentOrWhite(self),Transparency=.05}) end
			if self._verBadgeDot then tw(self._verBadgeDot,.14,{Size=UDim2.fromOffset(6,6)}) end
		end)
		badgeButton.MouseLeave:Connect(function()
			tw(self._verBadge,.14,{BackgroundColor3=C.Card3})
			if self._verBadgeLabel then tw(self._verBadgeLabel,.14,{TextColor3=C.TextDim}) end
			if self._verBadgeStroke then tw(self._verBadgeStroke,.14,{Color=fromHex("303030"),Transparency=.25}) end
			if self._verBadgeDot then tw(self._verBadgeDot,.14,{Size=UDim2.fromOffset(5,5)}) end
		end)
		badgeButton.Activated:Connect(function()
			if self._updateLogPageIndex then self:SetPage(self._updateLogPageIndex) end
		end)
	end
	...
	local root = new("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,LayoutOrder=self:_o(pageIndex)},scroll)
	vlist(root,14)

	local header = new("Frame",{Size=UDim2.new(1,0,0,66),BackgroundTransparency=1,LayoutOrder=0},root)
	local accent = new("Frame",{Position=UDim2.fromOffset(0,7),Size=UDim2.fromOffset(3,42),BackgroundColor3=accentOrWhite(self),BorderSizePixel=0},header)
	corner(accent,3)
	new("TextLabel",{Text=opts.Title or "Update Log",Font=Enum.Font.GothamBold,TextSize=21,TextColor3=C.White,BackgroundTransparency=1,Position=UDim2.fromOffset(14,0),Size=UDim2.new(1,-14,0,28),TextXAlignment=Enum.TextXAlignment.Left},header)
	new("TextLabel",{Text=opts.Subtitle or "Latest changes for this script",Font=Enum.Font.Gotham,TextSize=12,TextColor3=C.TextDim,BackgroundTransparency=1,Position=UDim2.fromOffset(14,30),Size=UDim2.new(1,-14,0,18),TextXAlignment=Enum.TextXAlignment.Left},header)
	new("TextLabel",{Text="SELECT A RELEASE",Font=Enum.Font.GothamBold,TextSize=9,TextColor3=C.TextOff,BackgroundTransparency=1,Position=UDim2.fromOffset(14,51),Size=UDim2.new(1,-14,0,14),TextXAlignment=Enum.TextXAlignment.Left},header)

	local function blinkCursor(label, baseText, attrName, gen, cycles, interval)
		cycles = cycles or 8
		interval = interval or 0.34
		for i = 1, cycles do
			if not label.Parent or label:GetAttribute(attrName) ~= gen then return end
			label.Text = baseText .. ((i % 2 == 1) and "|" or "")
			task.wait(interval)
		end
		if label.Parent and label:GetAttribute(attrName) == gen then
			label.Text = baseText
		end
	end
	...
	local function typewrite(label, text, speed)
		...
		local gen = (label:GetAttribute("TypeGen") or 0) + 1
		label:SetAttribute("TypeGen", gen)
		task.spawn(function()
			label.Text = "|"
			task.wait(0.12)
			for i = 1, #text do
				if not label.Parent or label:GetAttribute("TypeGen") ~= gen then return end
				label.Text = text:sub(1, i) .. "|"
				task.wait(speed or 0.018)
			end
			blinkCursor(label, text, "TypeGen", gen, 8, 0.28)
		end)
	end

	local function rewriteSelected(label, version, speed)
		if not label then return end
		local prefix = "SELECTED: "
		version = string.upper(tostring(version or ""))
		local gen = (label:GetAttribute("RewriteGen") or 0) + 1
		label:SetAttribute("RewriteGen", gen)
		task.spawn(function()
			local old = tostring(label:GetAttribute("SelectedVersion") or "")
			if old == "" then
				local current = tostring(label.Text or "")
				old = current:gsub("^" .. prefix, ""):gsub("|", "")
			end
			label.Text = prefix .. old .. "|"
			task.wait(0.08)
			for i = #old, 0, -1 do
				if not label.Parent or label:GetAttribute("RewriteGen") ~= gen then return end
				label.Text = prefix .. old:sub(1, i) .. "|"
				task.wait(speed or 0.012)
			end
			task.wait(0.06)
			for i = 1, #version do
				if not label.Parent or label:GetAttribute("RewriteGen") ~= gen then return end
				label.Text = prefix .. version:sub(1, i) .. "|"
				task.wait(speed or 0.014)
			end
			label:SetAttribute("SelectedVersion", version)
			blinkCursor(label, prefix .. version, "RewriteGen", gen, 6, 0.28)
		end)
	end

	local tabRow = new("ScrollingFrame",{Size=UDim2.new(1,0,0,42),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=0,CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.X,ScrollingDirection=Enum.ScrollingDirection.X,LayoutOrder=1},root)
	hlist(tabRow,10)

	local panel = new("Frame",{Size=UDim2.new(1,0,0,372),BackgroundColor3=fromHex("030303"),BorderSizePixel=0,LayoutOrder=2,ClipsDescendants=true},root)
	corner(panel,8)
	stroke(panel,fromHex("2f2f2f"),1,0)

	local panelHeader = new("Frame",{Size=UDim2.new(1,0,0,38),BackgroundColor3=fromHex("0c0c0c"),BorderSizePixel=0},panel)
	new("Frame",{Position=UDim2.new(0,0,1,-1),Size=UDim2.new(1,0,0,1),BackgroundColor3=C.Border,BorderSizePixel=0},panelHeader)
	local dots = new("Frame",{Position=UDim2.fromOffset(14,0),Size=UDim2.fromOffset(54,38),BackgroundTransparency=1},panelHeader)
	hlist(dots,7)
	for _, col in ipairs({C.Red, C.Yellow, C.Green}) do
		local d = new("Frame",{Size=UDim2.fromOffset(8,8),BackgroundColor3=col,BorderSizePixel=0},dots)
		corner(d,8)
	end
	local terminalTitle = new("TextLabel",{Text="",Font=Enum.Font.Code,TextSize=11,TextColor3=C.TextDim,BackgroundTransparency=1,Position=UDim2.fromOffset(84,0),Size=UDim2.new(1,-250,1,0),TextXAlignment=Enum.TextXAlignment.Left},panelHeader)
	local selectedLbl = new("TextLabel",{Text="",Font=Enum.Font.Code,TextSize=10,TextColor3=C.TextDim,BackgroundTransparency=1,AnchorPoint=Vector2.new(1,0),Position=UDim2.new(1,-14,0,0),Size=UDim2.fromOffset(230,38),TextXAlignment=Enum.TextXAlignment.Right},panelHeader)
	self._updateLogTitlePlayed = false
	self._playUpdateLogTitle = function()
		if self._updateLogTitlePlayed then return end
		if not terminalTitle or not terminalTitle.Parent then return end
		if self._bootSplash then return end
		self._updateLogTitlePlayed = true
		typewrite(terminalTitle, "BIOME SCRIPT CHANGELOG", 0.018)
	end

	local logScroll = new("ScrollingFrame",{Position=UDim2.fromOffset(0,38),Size=UDim2.new(1,0,1,-38),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=5,ScrollBarImageColor3=C.Border3,ScrollBarImageTransparency=.2,CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,ElasticBehavior=Enum.ElasticBehavior.Never},panel)
	pad(logScroll,16,18,16,16)
	local logLayout = new("UIListLayout",{SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,8)},logScroll)
	...
	local function clearLog()
		for _, child in ipairs(logScroll:GetChildren()) do
			if child ~= logLayout and not child:IsA("UIPadding") then child:Destroy() end
		end
	end

	local function makeText(parent, text, style, order)
		style = style or {}
		return new("TextLabel",{Text=tostring(text or ""),Font=style.Bold and Enum.Font.GothamBold or Enum.Font.Code,TextSize=style.Size or 13,TextColor3=style.Color or C.Text,BackgroundTransparency=1,Size=UDim2.new(0,0,0,20),AutomaticSize=Enum.AutomaticSize.XY,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Top,RichText=true,LayoutOrder=order or 0},parent)
	end

	local function addLine(line, order)
		local text, color, bold
		if type(line) == "table" then
			text = tostring(line.Text or "")
			color = line.Color or C.Text
			bold = line.Bold == true
		else
		...
		local row = new("Frame",{Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,LayoutOrder=order},logScroll)
		hlist(row,6,Enum.VerticalAlignment.Top)
		new("TextLabel",{Text=">",Font=Enum.Font.Code,TextSize=13,TextColor3=accentOrWhite(self),BackgroundTransparency=1,Size=UDim2.fromOffset(12,20),TextXAlignment=Enum.TextXAlignment.Left,LayoutOrder=0},row)

		if not text:find("#[%w_]+") then
			new("TextLabel",{Text=text,Font=bold and Enum.Font.GothamBold or Enum.Font.Code,TextSize=13,TextColor3=color,BackgroundTransparency=1,Size=UDim2.new(1,-20,0,0),AutomaticSize=Enum.AutomaticSize.Y,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Top,TextWrapped=true,RichText=true,LayoutOrder=1},row)
			return
		end

		local cursor = 1
		local pieceOrder = 0
		for startPos, tag, endPos in text:gmatch("()#([%w_]+)()") do
			local before = text:sub(cursor,startPos-1)
			if before ~= "" then
				pieceOrder += 1
				makeText(row,before,{Color=color,Bold=bold},pieceOrder + 1)
			end
			pieceOrder += 1
			local btn = new("TextButton",{Text="#" .. tag,Font=Enum.Font.GothamBold,TextSize=11,TextColor3=C.White,BackgroundColor3=fromHex("171717"),BorderSizePixel=0,Size=UDim2.new(0,0,0,22),AutomaticSize=Enum.AutomaticSize.X,LayoutOrder=pieceOrder + 1,AutoButtonColor=false},row)
			corner(btn,7)
			local pillStroke = stroke(btn,fromHex("343434"),1,.1)
			pad(btn,0,0,9,9)
			btn.MouseEnter:Connect(function()
				tw(btn,.12,{BackgroundColor3=fromHex("252525"),TextColor3=accentOrWhite(self)})
				if pillStroke then tw(pillStroke,.12,{Color=accentOrWhite(self),Transparency=.05}) end
			end)
			btn.MouseLeave:Connect(function()
				tw(btn,.12,{BackgroundColor3=fromHex("171717"),TextColor3=C.White})
				if pillStroke then tw(pillStroke,.12,{Color=fromHex("343434"),Transparency=.1}) end
			end)
			btn.Activated:Connect(function()
				local idx = self:FindPageIndex(tag)
				if idx then self:SetPage(idx) end
			end)
			cursor = endPos
		end
		local rest = text:sub(cursor)
		if rest ~= "" or pieceOrder == 0 then makeText(row,rest,{Color=color,Bold=bold},pieceOrder+2) end
	end

	function obj:SetVersion(version)
		if not version then return end
		activeVersion = version
		for ver, tab in pairs(tabButtons) do
			local active = ver == version
			tw(tab.Frame,.2,{BackgroundColor3=active and C.White or C.Card,Size=UDim2.fromOffset(tab.Width,active and 34 or 30)})
			tw(tab.Label,.2,{TextColor3=active and C.Bg or C.TextDim,TextSize=active and 12 or 11})
			if tab.Stroke then
				tw(tab.Stroke,.2,{Color=active and accentOrWhite(self) or fromHex("303030"),Transparency=active and .05 or .28})
			end
		end
		rewriteSelected(selectedLbl, version, 0.012)
		clearLog()
		for _, entry in ipairs(logs) do
			if entry.Version == version then
				for i, line in ipairs(validateTable(entry.Lines, {})) do addLine(line,i) end
				break
			end
		end
		task.defer(function() if logScroll.Parent then logScroll.CanvasPosition = Vector2.new(0,0) end end)
	end

	for i, entry in ipairs(logs) do
		local version = tostring(entry.Version or ("v" .. tostring(i)))
		entry.Version = version
		local tabW = math.max(78,#version*9+32)
		local btn = new("TextButton",{Text="",BackgroundColor3=i == 1 and C.White or C.Card,BorderSizePixel=0,Size=UDim2.fromOffset(tabW,i == 1 and 34 or 30),LayoutOrder=i,AutoButtonColor=false,ClipsDescendants=true},tabRow)
		corner(btn,7)
		local tabStroke = stroke(btn,i == 1 and accentOrWhite(self) or fromHex("303030"),1,i == 1 and .05 or .28)
		local lbl = new("TextLabel",{Text=version,Font=Enum.Font.GothamBold,TextSize=i == 1 and 12 or 11,TextColor3=i == 1 and C.Bg or C.TextDim,BackgroundTransparency=1,Size=UDim2.fromScale(1,1),TextXAlignment=Enum.TextXAlignment.Center,TextYAlignment=Enum.TextYAlignment.Center},btn)
		tabButtons[version] = {Frame=btn,Label=lbl,Stroke=tabStroke,Width=tabW}
		btn.MouseEnter:Connect(function()
			if activeVersion ~= version then
				tw(btn,.14,{BackgroundColor3=C.Card2,Size=UDim2.fromOffset(tabW,33)})
				tw(lbl,.14,{TextColor3=C.White,TextSize=12})
				if tabStroke then tw(tabStroke,.14,{Color=fromHex("4a4a4a"),Transparency=.08}) end
			end
		end)
		btn.MouseLeave:Connect(function()
			if activeVersion ~= version then
				tw(btn,.14,{BackgroundColor3=C.Card,Size=UDim2.fromOffset(tabW,30)})
				tw(lbl,.14,{TextColor3=C.TextDim,TextSize=11})
				if tabStroke then tw(tabStroke,.14,{Color=fromHex("303030"),Transparency=.28}) end
			end
		end)
		btn.Activated:Connect(function() obj:SetVersion(version) end)
	end

	if logs[1] then obj:SetVersion(logs[1].Version) end
